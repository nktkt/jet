...
- [ ] **0.5.3**: `[workspace.package]` field inheritance (`version.workspace = true`, `java.workspace = true`, etc.) — defers because PackageMeta's concrete-typed fields (`String`, `u32`) need a custom serde wrapper or a parallel `RawPackageMeta` type, neither of which is on the critical path for the BOM-pattern that drives most JVM workspaces.
- [ ] **0.5.3**: Shared workspace-root `jet.lock` — currently each member writes its own when build/resolve runs from inside it. The workspace.dependencies table already gives version unification across members; lockfile sharing is now an optimization, not a correctness fix.
