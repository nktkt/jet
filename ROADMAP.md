...
- [ ] **0.5.4**: Shared workspace-root `jet.lock` — currently each member writes its own when build/resolve runs from inside it. The workspace.dependencies + workspace.package tables already give version + metadata unification across members; lockfile sharing is now an optimization, not a correctness fix.
